...
public class Project extends Model<Project> {
	...
	public List<Project> selectProjectById(long id) {
		String selectStr = "select p.id as ProjectID,  p.ProjectCN, p.ProjectEN, p.Category ,Applytime,RegisterNum,Introduction,"
				+ "Content ,Content1,Content2,Content3,Content4,AppSituation ,TotalFund,Payback,Calculations,Social,  "
				+ "(select Name from set_category sc where sc.id=p.Category) Category_name,"
				+
				...
				"(select Name from project_group sc where sc.id=p.GroupZY) Group_name,p.GroupZY,"
				+ "(case when p.Secret='1' then '是' else '否' end ) Secret_name,Secret,SecretTime,PlanFund,"
				+ "RecommendCompany,FromTask,FromOther,StartTime,EndTime from project p where id ="
				+ id;

		return find(selectStr);
	}
